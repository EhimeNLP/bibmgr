...
The `archive` profile is the database-ingest boundary. It preserves field spelling and order, has no required or forbidden fields, disables every `LAB-*` convention, accepts unresolved semantics, and leaves only strict parser diagnostics blocking. It never applies fixes. General analysis and export default to `modern`, which also disables `LAB-*` rules. The `laboratory` validation profile remains available for explicit linting and export requests; its citation-key, field spelling, field order, required-data, and representation rules do not decide whether source may be archived. No URL policy offers a metadata-deleting fix.
