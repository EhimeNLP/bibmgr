...
Each non-deleted revision stores the submitted BibTeX, canonical laboratory BibTeX, complete semantic snapshot, reference row, contributors, identifiers, URLs, and citation contexts. This lets an operator inspect the exact submitted entry while restoration reinstates the canonical current state. A delete revision stores the complete previous state and a null resulting state. Audit rows deliberately do not have a foreign key to the live reference because deletion must not erase or null the historical reference ID. PostgreSQL rejects updates and deletes against the audit table with an append-only trigger.
