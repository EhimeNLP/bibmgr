...
def test_cli_writes_only_validated_entries_and_separate_review_report(tmp_path):
    ...
    assert report_json["input_artifact_id"].startswith("sha256:")
    assert report_json["artifacts"]
    assert (tmp_path / "review-artifacts").is_dir()
...
def test_cli_produces_a_rust_validated_bibliography_without_network(tmp_path):
    ...
    orchestrator = ReconstructionOrchestrator(
        ...
        citation_client=NoCitationClient(),
    ...
    entries, report = reconstruct_file(
        ...
        key_generator=CitationKeyGenerator(
            concept_generator=RuleConceptGenerator(),
        ),
    )
    ...
    assert report_json["schema_version"] == "2"
    assert report_json["processed_references"][0][
        "final_artifact_id"
    ].startswith("sha256:")
...
def test_cli_writes_outputs_when_ready_entry_has_no_venue(tmp_path):
    input_path = tmp_path / "metadata.json"
    output_path = tmp_path / "reconstructed.bib"
    ...
    input_path.write_text(json.dumps({
        ...
        "references": [{
            ...
            "doi": None,
            "venue": None,
            "raw_text": "Jacob Devlin. 2019. BERT.",
        }],
    }), encoding="utf-8")
    ...
    entries, report = reconstruct_file(
        ...
        output_path,
        report_path,
        orchestrator=NoVenueOrchestrator(),
        key_generator=CitationKeyGenerator(
            concept_generator=RuleConceptGenerator(),
        ),
    )
    ...
    assert "devlin-2019-unknown-bert" in output_path.read_text(
        encoding="utf-8"
    )
    assert report_path.is_file()
    assert (tmp_path / "report-artifacts").is_dir()
