...
from .application.artifacts import ArtifactStore, reconstruction_run
...
def reconstruct_file(
    ...
    *,
    ...
    artifact_root = artifact_directory or report_path.with_name(
        f"{report_path.stem}-artifacts"
    )
    artifact_store = ArtifactStore(artifact_root)
    input_artifact_id = artifact_store.add_bytes(
        input_bytes,
        media_type="application/json",
        suffix=".json",
    )
    for result in processed:
        artifact_store.capture_reference(result)
    report = ReconstructionReport(
        ...
        artifacts=artifact_store.references,
