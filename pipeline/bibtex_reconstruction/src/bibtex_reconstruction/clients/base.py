...
class BaseAPIClient(ABC):
    ...
    def request_retries(self) -> int:
        """Return a provider override or the shared retry count."""

        return getattr(
            settings,
            f"{self.api_prefix}_max_retries",
            settings.max_retries,
        )

    @property
    def authoritative_bibtex(self) -> bool:
        """Whether the export is authoritative for the represented record."""
        return False

    @property
    def direct_bibtex_eligible(self) -> bool:
        """Whether a provider-returned BibTeX may enter validated selection."""

        return self.authoritative_bibtex
    ...
    def search(self, input_data: InputData) -> Tuple[Optional[VerifiedCitationInfo], Optional[str]]:
        ...
        try:
            ...
            # Keep provider evidence independent. DOI content negotiation and
            # any deterministic supplementation are orchestrator concerns and
            # must never be disguised as this provider's own export.
        ...
        for attempt_index in range(retries):
            ...
            try:
                response = rate_limiter.call(
                    ...
                    error_cooldown=self._retry_delay(attempt),
                )
