...
from ..parsing.bibtex import bibtex_fields
...
class ArxivClient(BaseAPIClient):
    ...
    def _execute_search(
        ...
        explicit_id = self.explicit_arxiv_id(input_data)
        if explicit_id:
            bibtex = self._fetch_official_bibtex(explicit_id)
            metadata = self._metadata_from_bibtex(explicit_id, bibtex)
    ...
    @staticmethod
    def explicit_arxiv_id(input_data: InputData) -> str | None:
        """Return an arXiv identifier explicitly present in search evidence."""

        reference = input_data.parsed_data
        for value in (reference.title, reference.raw_text, reference.doi):
            match = _EXPLICIT_ARXIV_ID.search(value or "")
            if match:
                return match.group(1)
        return None

    @staticmethod
    def _metadata_from_bibtex(
        arxiv_id: str,
        bibtex: str | None,
    ) -> VerifiedCitationInfo | None:
        fields = bibtex_fields(bibtex or "")
